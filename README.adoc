...
#PIERRE Elona# et #PELOILLE Mathéo#
...
Le code est structuré en couches : TodoItem est la couche infrastructure, TodoListController est la couche domaine.
Le domaine est donc directement dépendant de l'infrastructure car l'infrastructure a sûrement été  définie avant le domaine.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se trouve dans TodoListController.

*Cette règle est-elle facilement testable par un test unitaire ?*
Non car elle dépend du temps qui est une donnée mouvante.
...
Cette commande permet de lancer une base PostgreSQL en Docker :
- docker run : lancer un nouveau conteneur en docker
- -it : mode interactif
- -e : définition des variables d'environnement POSTGRES_PASSWORD=password
- -p 5432:5432 : mapping des ports
- postgres : nom de officiel de l'image

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
todoItemRepository est null car il n'est pas injecté dans la classe TodoListController.
La meilleure façon de l'injecter est de le faire dans le constructeur de la classe TodoListController.
...
Dans l'architecture hexagonale, le fonctionnel est au milieu (domaine) et le technique dans des adaptateurs.
Le domaine métier est entouré de ports qui sont implémentés par des adaptateurs.
Les ports sont des portes d'entrées et de sorties vers le domaine.
Cela permet de réduire le couplage.
Pour respecter l'architecture, il ne doit pas y avoir d'appels entre adaptateurs et d'appels entre le domaine et les adaptateurs.
...
Pour tester les adaptateurs, il faut faire des tests d'intégration.
